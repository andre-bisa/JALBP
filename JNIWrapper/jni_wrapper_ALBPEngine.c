...
static jint c_register(JNIEnv *env, jobject obj, jobject ALBPSocket_, jstring dtnDemuxString, jint IPNDemux) {
	...
	jmethodID mid = (*env)->GetMethodID(env, ALBPSocket, "setRegistrationDescriptor", "(I)V");
...
static boolean_t copy_bundle_to_java(JNIEnv *env, jobject ALBPBundle_, al_bp_bundle_object_t bundle, al_bp_bundle_payload_location_t payloadLocation) {
	...
	// BLOCKS
	mid = (*env)->GetMethodID(env, ALBPBundle, "addBlock", "([BII)V");
	CHECKMIDNOTNULL(mid);
	for(i = 0; i < bundle.spec->blocks.blocks_len; i++) {
	...
	mid = (*env)->GetMethodID(env, ALBPBundle, "setUnreliable", "(I)V");
	CHECKMIDNOTNULL(mid);
	(*env)->CallVoidMethod(env, ALBPBundle_, mid, (jint) bundle.spec->unreliable);
	
	mid = (*env)->GetMethodID(env, ALBPBundle, "setCritical", "(I)V");
	CHECKMIDNOTNULL(mid);
	(*env)->CallVoidMethod(env, ALBPBundle_, mid, (jint) bundle.spec->critical);
	
	mid = (*env)->GetMethodID(env, ALBPBundle, "setFlowLabel", "(I)V");
	CHECKMIDNOTNULL(mid);
	...
	switch (payloadLocation) {
		case BP_PAYLOAD_MEM:
			mid = (*env)->GetMethodID(env, ALBPBundle, "setBuffer", "([B)V");
			CHECKMIDNOTNULL(mid);
		...
		case BP_PAYLOAD_FILE:
			mid = (*env)->GetMethodID(env, ALBPBundle, "setPayloadFile", "(Ljava/lang/String;)V");
			CHECKMIDNOTNULL(mid);
			(*env)->CallVoidMethod(env, ALBPBundle_, mid, (*env)->NewStringUTF(env, bundle.payload->filename.filename_val));
			break;
			
		case BP_PAYLOAD_TEMP_FILE:
			mid = (*env)->GetMethodID(env, ALBPBundle, "setPayloadTemporaryFile", "(Ljava/lang/String;)V");
			CHECKMIDNOTNULL(mid);
			(*env)->CallVoidMethod(env, ALBPBundle_, mid, (*env)->NewStringUTF(env, bundle.payload->filename.filename_val));
			break;
	...
	if (bundle.payload->status_report != NULL) {
			...
			mid = (*env)->GetMethodID(env, ALBPBundle, "setSourceStatusReport", "(Ljava/lang/String;)V");
			CHECKMIDNOTNULL(mid);
			(*env)->CallVoidMethod(env, ALBPBundle_, mid, (*env)->NewStringUTF(env, bundle.payload->status_report->bundle_id.source.uri));
			
			mid = (*env)->GetMethodID(env, ALBPBundle, "setCreationTimestampStatusReport", "(II)V");
			CHECKMIDNOTNULL(mid);
			...
			mid = (*env)->GetMethodID(env, ALBPBundle, "setCustodyTimestampStatusReport", "(II)V");
			CHECKMIDNOTNULL(mid);
			...
			mid = (*env)->GetMethodID(env, ALBPBundle, "setForwardingTimestampStatusReport", "(II)V");
			CHECKMIDNOTNULL(mid);
			...
			mid = (*env)->GetMethodID(env, ALBPBundle, "setDeliveryTimestampStatusReport", "(II)V");
			CHECKMIDNOTNULL(mid);
			...
			mid = (*env)->GetMethodID(env, ALBPBundle, "setDeletionTimestampStatusReport", "(II)V");
			CHECKMIDNOTNULL(mid);
			...
			mid = (*env)->GetMethodID(env, ALBPBundle, "setAckByAppTimestampStatusReport", "(II)V");
			CHECKMIDNOTNULL(mid);
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	for (int i = 0; i < albpJNIWrapperLinkerDim; i++) {
		jclass  cls;
		jint    res;
